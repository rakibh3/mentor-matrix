...
import { SessionMonitor } from '@/components/admin/overview/SessionMonitor';
import { AnalyticsSnapshot } from '@/components/admin/overview/AnalyticsSnapshot';
import { FlaggedStudentsList } from '@/components/admin/overview/FlaggedStudentsList';
import { FlaggedStudent, RiskLevel, CallOutcome } from '@/types';
...
// Modals
import { BulkAbsentModal } from '@/components/admin/modal/BulkAbsentModal';
...
const AdminDashboard: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  const hasActiveFilters = searchQuery !== '' || assignmentFilter !== 'Assignments';
  ...
  const editStudentMutation = useMutation({
    mutationFn: async (updated: FlaggedStudent) => updated,
    onSuccess: (data) => {
      queryClient.setQueryData(['flaggedStudents'], (old: FlaggedStudent[] | undefined) => {
        return old?.map(s => s.id === editingStudent?.id ? { ...s, ...data } : s);
      });
      setEditingStudent(null);
      addToast({ type: 'success', title: 'Profile Updated', message: 'Student information has been successfully synchronized.' });
    }
  });
  ...
  const deleteStudentMutation = useMutation({
    ...
    onSuccess: (id) => {
      ...
      addToast({ type: 'info', title: 'Record Removed', message: 'Student has been removed from the monitoring list.' });
    }
  });
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-text-secondary text-base font-medium flex items-center gap-2">
              <span className="size-2 rounded-full bg-primary animate-pulse shadow-[0_0_8px_#13ec6a]"></span>
