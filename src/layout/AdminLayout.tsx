...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ onLogout, isCollapsed, onToggle }) => {
  ...
  return (
    ...
    >
      ...
      <button 
        onClick={onToggle}
        className="absolute -right-4 top-10 size-8 bg-surface-dark border border-card-border rounded-full flex items-center justify-center text-text-secondary hover:text-primary transition-all shadow-xl z-50 group active:scale-90"
      >
        <Icon 
      ...
      <div className="flex flex-col gap-10">
        <div className={`flex items-center gap-3 px-1 transition-all duration-300 ${isCollapsed ? 'justify-center' : ''}`}>
          <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-primary/10 border border-primary/20 text-primary shadow-[0_0_15px_rgba(19,236,106,0.1)]">
      ...
      <div className={`flex items-center gap-3 p-3 bg-surface-dark/30 border border-card-border/40 rounded-2xl transition-all duration-300 ${isCollapsed ? 'flex-col justify-center' : 'justify-between'}`}>
        <div className={`flex items-center gap-3 overflow-hidden ${isCollapsed ? 'flex-col' : ''}`}>
          <div className="size-10 rounded-xl bg-primary/20 flex-shrink-0 flex items-center justify-center text-primary border border-primary/10">
        ...
        <button 
          onClick={onLogout}
          className={`p-2 text-text-secondary hover:text-red-400 transition-all active:scale-90 rounded-lg hover:bg-red-500/5 ${isCollapsed ? 'w-full flex justify-center' : ''}`}
          title="Logout"
        >
      ...
      {mobileMenuOpen && (
        ...
        >
          <div className="absolute left-0 top-0 bottom-0 w-64 bg-background-dark border-r border-border-dark p-6 animate-in slide-in-from-left duration-300">
            ...
            <button 
              onClick={onLogout}
              className="mt-auto flex items-center gap-3 px-4 py-3 rounded-xl w-full text-red-500 hover:bg-red-500/10 transition-all font-bold mt-10"
            >
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden relative border-l border-border-dark/10">
        <header className="md:hidden flex items-center justify-between px-6 py-4 bg-background-dark border-b border-border-dark z-50">
          ...
          <div className="flex items-center gap-4">
            <button className="text-white p-2 hover:bg-white/5 rounded-xl transition-all" onClick={() => setMobileMenuOpen(true)}>
...
export const SettingsNav = () => {
  ...
  return (
    <div className="flex overflow-x-auto gap-2 border-b border-card-border mb-8 scrollbar-none">
      {tabs.map(tab => (
        <Link 
          key={tab.path}
          to={tab.path}
          className={`px-8 py-4 text-xs font-black uppercase tracking-widest whitespace-nowrap transition-all border-b-2 ${location.pathname === tab.path ? 'text-primary border-primary bg-primary/5' : 'text-text-secondary border-transparent hover:text-white hover:bg-white/5'}`}
        >
          {tab.label}
        </Link>
      ))}
